...
DOCKER_IMAGE=maurosoft1973/alpine-mariadb
RELEASE=TEST
...
do
    case $arg in
        ...
        echo -e "  -mv=|--mariadb-version      -> ${MARIADB_VERSION:-""} (mariadb version)"
...
if [ "${MARIADB_VERSION}" == "" ]; then
    echo "ERROR: The variable MARIADB_VERSION is not set!"
    ARGUMENT_ERROR=1
fi
...
if [ "$TOKEN" != "null" ]; then
    ...
    if [ "$RELEASE" == "TEST" ]; then
        ...
        docker manifest create ${DOCKER_IMAGE}:${ALPINE_VERSION}-${MARIADB_VERSION} \
            --amend ${DOCKER_IMAGE}:${ALPINE_VERSION}-${MARIADB_VERSION}-aarch64 \
            --amend ${DOCKER_IMAGE}:${ALPINE_VERSION}-${MARIADB_VERSION}-armhf \
            --amend ${DOCKER_IMAGE}:${ALPINE_VERSION}-${MARIADB_VERSION}-armv7 \
            --amend ${DOCKER_IMAGE}:${ALPINE_VERSION}-${MARIADB_VERSION}-ppc64le \
            --amend ${DOCKER_IMAGE}:${ALPINE_VERSION}-${MARIADB_VERSION}-x86 \
            --amend ${DOCKER_IMAGE}:${ALPINE_VERSION}-${MARIADB_VERSION}-x86_64

        docker manifest push ${DOCKER_IMAGE}:${ALPINE_VERSION}-${MARIADB_VERSION}
    else
        echo "Push Manifest for $RELEASE"
        ...
        docker manifest create ${DOCKER_IMAGE}:${MARIADB_VERSION} \
            --amend ${DOCKER_IMAGE}:${MARIADB_VERSION}-aarch64 \
            --amend ${DOCKER_IMAGE}:${MARIADB_VERSION}-armhf \
            --amend ${DOCKER_IMAGE}:${MARIADB_VERSION}-armv7 \
            --amend ${DOCKER_IMAGE}:${MARIADB_VERSION}-ppc64le \
            --amend ${DOCKER_IMAGE}:${MARIADB_VERSION}-x86 \
            --amend ${DOCKER_IMAGE}:${MARIADB_VERSION}-x86_64

        docker manifest push ${DOCKER_IMAGE}:${MARIADB_VERSION}
    fi
else 
