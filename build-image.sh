...
do
    case $arg in
        ...
        shift # Remove
        ;;
        -md=*|--mariadb-version-date=*)
        MARIADB_VERSION_DATE="${arg#*=}"
        shift # Remove
        ;;
        -r=*|--release=*)
        RELEASE="${arg#*=}"
        ...
        echo -e "  -md=|--mariadb-version-date -> ${MARIADB_VERSION_DATE:-""} (mariadb version date)"
...
if [ "${MARIADB_VERSION_DATE}" == "" ]; then
    echo "ERROR: The variable MARIADB_VERSION_DATE is not set!"
    ARGUMENT_ERROR=1
fi
...
if [ "$RELEASE" == "TEST" ]; then
    ...
    docker buildx build --platform ${PLATFORM} \
            ...
            --build-arg MARIADB_VERSION_DATE="${MARIADB_VERSION_DATE}" \
    ...
    echo "Remove image ${DOCKER_IMAGE}:${ALPINE_VERSION}-${MARIADB_VERSION}-${ALPINE_ARCHITECTURE}"
    docker rmi -f ${DOCKER_IMAGE}:${ALPINE_VERSION}-${MARIADB_VERSION}-${ALPINE_ARCHITECTURE}> /dev/null 2>&1
    ...
    docker buildx build --platform ${PLATFORM} \
            ...
            -t ${DOCKER_IMAGE}:${ALPINE_VERSION}-${MARIADB_VERSION}-${ALPINE_ARCHITECTURE} \
            -f ./Dockerfile .
else
    echo "Remove image ${DOCKER_IMAGE}:${ALPINE_ARCHITECTURE}"
    docker rmi -f ${DOCKER_IMAGE}:${ALPINE_ARCHITECTURE} > /dev/null 2>&1

    echo "Remove image ${DOCKER_IMAGE}:${MARIADB_VERSION}-${ALPINE_ARCHITECTURE}"
    docker rmi -f ${DOCKER_IMAGE}:${MARIADB_VERSION}-${ALPINE_ARCHITECTURE}> /dev/null 2>&1
    ...
    docker buildx build --platform ${PLATFORM} \
            ...
            -t ${DOCKER_IMAGE}:${MARIADB_VERSION}-${ALPINE_ARCHITECTURE} \
            -f ./Dockerfile .
fi
